...
(function ($, window) {
            ...
            var fn = {
                f_init: function () {
                    ...
                    $('#f_out').on('click', function () {
                        $('#contentIn', parent.window.document).attr('src', Carnet.basePrefixURL + '/static/wangyueche/html/map/abnormalFence.html');
                    });
                    // -----------------------添加功能
                    if (me.fn == 'add') {
                        me._tool();
                ...
                _tool_draw: function () {
                    ...
                    me.mouse_tools.on('draw', function (data) {
                        ...
                        _dom.name = me.name;
                        _dom.memo = me.memo;
                        _dom.radius = me.radius;
                        _dom.firstNum = me.firstNum;
                        _dom.secNum = me.secNum;
                        _dom.thrNum = me.thrNum;
                        _dom.type = me.type;
                        ...
                        if (me.radius) {
                            var circle = new AMap.Circle({   //圆形编辑器的样式
                                map: me.map,
                                center: new AMap.LngLat(me._dom.getCenter().lng, me._dom.getCenter().lat),
                                radius: me.radius,
                            ...
                            me._dom = circle;
                            me._dom.name = me.name;
                            me._dom.memo = me.memo;
                            me._dom.radius = me.radius;
                            me._dom.firstNum = me.firstNum;
                            me._dom.secNum = me.secNum;
                            me._dom.thrNum = me.thrNum;
                            me._dom.type = me.type;
                ...
                _save_yuan: function () {
                    ...
                    return opts = {
                        ...
                        lat: c.lat,
                        radius: r,
                        firstNum: me._dom.firstNum,
                        secNum: me._dom.secNum,
                        thrNum: me._dom.thrNum,
                        type: me._dom.type,
                ...
                _show_yuan: function (data) {
                    ...
                    marker.memo = data.memo;
                    marker.firstNum = data.firstNum;
                    marker.secNum = data.secNum;
                    marker.thrNum = data.thrNum;
                    marker.type = data.type;


                    // --------------编辑模式
                    if (me.fn == 'upd') {
                        if (me.fence_id == marker.id) {
                            me._dom = marker;
                            me.mode = new AMap.CircleEditor(me.map, me._dom);
                            me.mode.open();
                            me._dom.setOptions(me.editOptions);
                ...
                _show_duo: function (data) {
                    ...
                    Polygon.firstNum = data.firstNum;
                    Polygon.secNum = data.secNum;
                    ...
                    for (var i = 0; i < arr.length; i++) {
                        ...
                        arr[i].on('mouseover', function (e) {
                            e.target.indexLayer = layer.msg('名称：' + e.target.name +
                                '</br>备注：' + e.target.memo +
                                '</br>一级：' + e.target.firstNum +
                                '</br>二级：' + e.target.secNum +
                                '</br>三级：' + e.target.thrNum, {
                                time: 0, //不自动关闭
                            });
                        });
                        arr[i].off('mouseout');
                ...
                _add: function () {
                    ...
                    var str = '' +
                        ...
                        '</p>' +
                        '<p id = "s_radius" class="f_p">' +
                        '<span> 半径： </span>' +
                        '<input name = "type" type = "text"  id = "radius"/> *' +
                        '</p>' +
                        '<p id = "type" class="f_p">' +
                        '<span> 描述： </span>' +
                        '<input name = "type" type = "text"  id = "memo"/>' +
                        '</p>' +
                        '<p class="f_p">' +
                        '<span> 一级： </span>' +
                        '<input name = "type" type = "text"  id = "firstNum" /> *' +
                        '</p>' +
                        '<p class="f_p">' +
                        '<span> 二级： </span>' +
                        '<input name = "type" type = "text"  id = "secNum"/> *' +
                        '</p>' +
                        '<p class="f_p">' +
                        '<span> 三级： </span>' +
                        '<input name = "type" type = "text"  id = "thrNum"/> *' +
                        '</p>';
                ...
                _add_yes: function (index) {
                    ...
                    function val() {
                        ...
                        if (isNaN(Number($('#firstNum').val()))) {
                            layer.msg('警告级别请输入数字！');
                            return;
                        }
                        if (isNaN(Number($('#secNum').val()))) {
                            layer.msg('警告级别请输入数字！');
                            return;
                        }
                        if (isNaN(Number($('#thrNum').val()))) {
                            layer.msg('警告级别请输入数字！');
                            return;
                        }
                    }
                    ...
                    me.firstNum = $('#firstNum').val();
                    me.secNum = $('#secNum').val();
                    me.thrNum = $('#thrNum').val();
                    me.type = "3";
                ...
                _upd_f: function (argument) {
                    ...
                    $('#f_upd_save').on('click', function (argument) {
                        ...
                        var str = '' +
                            ...
                            '<p id = "type" class="f_p">' +
                            '<span> 描述： </span>' +
                            '<input name = "type" type = "text"  id = "memo" value=' + dom.memo + ' >' +
                            '</p>' +
                            '<p class="f_p">' +
                            '<span> 一级： </span>' +
                            '<input name = "type" type = "text"  id = "firstNum" value=' + dom.firstNum + ' > *' +
                            '</p>' +
                            '<p class="f_p">' +
                            '<span> 二级： </span>' +
                            '<input name = "type" type = "text"  id = "secNum" value=' + dom.secNum + ' > *' +
                            '</p>' +
                            '<p class="f_p">' +
                            '<span> 三级： </span>' +
                            '<input name = "type" type = "text"  id = "thrNum" value=' + dom.thrNum + ' > *' +
                            '</p>';
                    ...
                    me.api.upd(opts)
                        .then(function (data) {
                            if (data > 0) {
                                layer.msg('修改围栏成功');
                                layer.close(index);
                                // $('#contentIn', parent.window.document).attr('src', Carnet.basePrefixURL + '/html/fence/abnormalFence.html');
                                $('#contentIn', parent.window.document).attr('src', Carnet.basePrefixURL + '/static/wangyueche/html/map/abnormalFence.html');
                            } else {
                                layer.msg('修改围栏失败');
                            }
    ...
    Carnet.module["no_GD_fence_set"] = no_GD_fence_set;
})(jQuery, window);
