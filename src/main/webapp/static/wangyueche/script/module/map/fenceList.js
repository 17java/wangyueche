            ...
            _self.listTypeShow = [[
                ...
                }},
                {
                    ...
                    formatter: function (value, row, index) {
                        // '<a class="smallBtn" target="_self" href="fence/map_fence_update.html?id="'+(row.id)+'>修改</a> &nbsp;'
                        var btn = "";
                        btn = (row.status==0? '<a class="smallBtn" onclick="fenceChangeStatus(' + row.id + ''+',1'+')" >启用</a>':'<a class="smallBtn" onclick="fenceChangeStatus(' + row.id + ''+',0'+')" >停用</a>')
                            + '&nbsp<a class="smallBtn" onclick="fenceDelete(' + row.id + ')">删除</a> &nbsp';
...
function fenceChangeStatus(id,status) {
    layer.confirm('您真的确定要'+(status==0?'停用':'启用')+'该围栏吗？', {icon: 3, title: '提示'}, function (index) {
        return $.ajax({
            url: "http://" + window.location.host + "/wangyueche/fence/changeStatus",
            dataType: "json",
