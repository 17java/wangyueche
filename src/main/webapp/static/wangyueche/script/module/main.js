    ...
    main.prototype = {
        ...
        getMenu: function () {
            ...
            if(userName == 'admin'){
                ...
                userId = 7;
            }else if(userName == 'yichang'){
                userId = 8;
            }else if(userName == 'test'){
                userId = 9;
            }

            _self.server.getUserMenu({userId:userId}).done(function (res) {
                if (res) {
                    _self.dataMenu = res;
                    $('#navUl').html('');
                    $.each(res, function (i) {
                    ...
                    if (data[i]['parentId'] == id) {
                        if (data[i]['url']) {
                            ...
                            $('#leftTab').append('<li dataid="' + data[i]['id'] + '">' +
                                ...
                                '<a  href="' + '/wangyueche/static/wangyueche/html' + data[i]['url'] + '.html' + '" dataLevel="second" target="downcon">' +
                    ...
                    (function (obj) {
                        $.each(data, function (m) {
                            if (data[m]['parentId'] == obj.attr('dataid')) {
                                $(obj).find('ul').append('<li><a href="' + '/wangyueche/static/wangyueche/html' + data[m]['url'] + '.html' + '" target="downcon"><span class="chcufa">' + data[m]['permissionName'] + '</span></a></li>')
                            }
                        })
                    })(obj)
                })
