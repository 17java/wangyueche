...
public class Pager implements Serializable {
    ...
    public Pager(int currentPage, int limit) {
        this(currentPage, limit, 0, 0);
    }

    public Pager(int currentPage, int limit, int sort, int order) {
    ...
    public Pager(int currentPage, int limit, int sort, int order, int sort2, int order2) {
        this.offset = 1 + (limit * (currentPage - 1));
        this.offset = (offset < 0)? 0 : offset;
        this.limit = (limit <= 0)? 20 : limit;
        this.intSort = sort;
        this.order = (order == DESC)? ORDERS[0] : ORDERS[1];
